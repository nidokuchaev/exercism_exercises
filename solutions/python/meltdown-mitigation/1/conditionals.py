...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    return 'green' if percentage_value >= 80 else 'orange' if percentage_value < 80 and percentage_value >= 60 else 'red' if percentage_value < 60 and percentage_value >= 30 else 'black'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    return 'LOW' if temperature * neutrons_produced_per_second < threshold *  0.9 else 'NORMAL' if temperature * neutrons_produced_per_second >= threshold * 0.9  and temperature * neutrons_produced_per_second <= threshold * 1.1 else 'DANGER'
